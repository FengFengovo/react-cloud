...
const PlayerCenter = ({ audioRef }) => {
  ...
  const likeSong = async () => {
    await likeSongAPI(songInfo.id, !loveStatus)
    setLoveStatus(!loveStatus)
  }
  return (
    <div className="flex items-center">
      <i className="iconfont text-25px mx-40px text-white hover:text-[#fa3d49] transition-all duration-300 hover:scale-110 cursor-pointer">
        &#xe68c;
      </i>
      <i
        className="iconfont text-30px text-white hover:text-[#fa3d49] transition-all duration-300 hover:scale-110 cursor-pointer"
        onClick={Prev}
      >
        &#xe63c;
      </i>
      ...
      <i
        className={classNames(
          "iconfont text-25px mx-40px text-gray-1 hover:text-[#fa3d49] transition-all duration-300 hover:scale-110 cursor-pointer",
          { "text-red-5": loveStatus },
        )}
